...
if platform.system() == "Darwin":
    ...
    try:
        glew_header = os.path.join(os.environ["GLEW_INCLUDE_DIR"], "GL", "glew.h")
        includes += [os.environ["GLEW_INCLUDE_DIR"], os.path.join(os.environ["GLEW_INCLUDE_DIR"], "GL")]
    except KeyError:
        glew_header = "/usr/include/GL/glew.h"
        includes += ["/usr/include/GL"]
    try:
        includes += [os.environ["GL_INCLUDE_DIR"]]
    except KeyError:
        pass
    try:
